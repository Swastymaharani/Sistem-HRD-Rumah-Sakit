...
@section('content')
    ...
    <div class="nk-fmg-quick-list nk-block">
        ...
        <form name="formPendaftaran" id="formPendaftaran" action="{{ route('authpegawai.update', auth()->user()->id)}}" method="POST"  enctype="multipart/form-data">
            @csrf
            {{-- @method('PUT') --}}
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3 row">
                        <div class="col-sm-5"><a title='Ubah Data' href='javascript:void(0)' onclick='update("","")' class='btn btn-primary'>Simpan</a></div>
                    </div>
                </div>
            </div>
        </form>
...
function update(){
    ...
    CustomSwal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url:"{{route('authpegawai.update', auth()->user()->id)}}/",
                data:formData,
                contentType: false,
                processData: false,
                type:"POST",
